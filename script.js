(function () {
  ...
  // ─── FIX #4: Deutsch — islamische Hijri-Monatsnamen, KEIN "v. Chr." ────────
  ...
  // Hijri-Locale-Map: bs und de werden manuell formatiert
  const HIJRI_LOCALE_MAP = {
    'bs': null,
    'de': null,   // FIX #4: manuell, damit kein "v. Chr." erscheint
  ...
  // ─── Bosnisch: Datum manuell formatieren ────────────────────────────────────
  ...
  // ─── Hijri-Datum: Zahlen aus EN holen, dann Monat übersetzen ────────────────
  ...
  function formatHijriBosnian(now) {
    ...
    if (!p) return '';
    const monthName = BS_HIJRI_MONTHS[p.monthIndex] || String(p.monthIndex + 1);
    return `${p.day}. ${monthName} ${p.year}. h.`;
  }

  // ─── FIX #4: Deutsches Hijri ohne "v. Chr." ─────────────────────────────────
  function formatHijriGerman(now) {
    const p = getHijriParts(now);
    if (!p) return '';
    const monthName = DE_HIJRI_MONTHS[p.monthIndex] || String(p.monthIndex + 1);
    return `${p.day}. ${monthName} ${p.year} n. H.`;
  }
  ...
  function runEngine() {
    ...
    if (hijriEl) {
      if (currentLang === 'bs') {
        ...
        // FIX #4: Deutsch — manuell, kein "v. Chr."
        hijriEl.textContent = formatHijriGerman(now);

      } else {
        // Arabisch, Türkisch — native Intl
    ...
    // FIX #5: Nächste Gebetsbox grün markieren
  ...
  function updateCountdown(now) {
    ...
    if (!nextPrayerDate) {
      ...
      if (nextPrayerDate) nextPrayerDate.setDate(nextPrayerDate.getDate() + 1);
    }
  ...
  // ─── FIX #5: Nächste Gebetsbox hervorheben ──────────────────────────────────
  // HTML-Voraussetzung: jede .prayer-group braucht data-prayer="fajr" etc.
  function highlightNextPrayer(now) {
    ...
    for (const p of prayerMap) {
      const pDate = parseTimeToDate(cachedPrayerTimes[p.idx], now);
      if (pDate && pDate > now) { nextKey = p.key; break; }
    }

    // Fajr des nächsten Tages wenn alle vorbei
    if (!nextKey) nextKey = 'fajr';

    document.querySelectorAll('.prayer-group').forEach(el => {
      if (el.dataset.prayer === nextKey) {
        el.classList.add('prayer-group--active');
      } else {
        el.classList.remove('prayer-group--active');
      }
    });
  }

  // ─── PWA Install Prompt (auf Gebetsseiten) ───────────────────────────────────
  // FIX #3: Auch auf bs.html / de.html etc. anbieten, wenn Banner-Element vorhanden
  ...
  if (dismissBtn) {
    dismissBtn.addEventListener('click', () => {
      ...
      localStorage.setItem('pwa_dismissed', expires);
    });
  }
